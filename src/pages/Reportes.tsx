...
import { FileText, AlertCircle, Inbox, Phone, ArrowLeft, RefreshCw } from "lucide-react";
          ...
          <div className="max-w-3xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-muted-foreground">Listado público de los reportes registrados.</p>
              </div>
              <button onClick={load} disabled={status === "loading"} className="btn btn-secondary btn-sm gap-2 self-start">
            ...
            {status === "error" && (
              <div role="alert" className="flex flex-col items-center text-center gap-3 rounded-lg border border-red-200 bg-red-50 p-8 text-red-800">
                <AlertCircle className="w-8 h-8" />
                <p className="font-semibold">No se pudieron cargar los reportes.</p>
                <p className="text-sm">{errorMsg}</p>
                <button onClick={load} className="btn btn-secondary btn-sm mt-2">Reintentar</button>
              </div>
            )}
            ...
            {status === "success" && fraudes.length === 0 && (
              <div className="flex flex-col items-center text-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-12 text-muted-foreground">
                ...
                <p className="font-semibold text-foreground">Aún no hay reportes registrados.</p>
                ...
                {fraudes.map((fraude) => (
                  <article key={fraude.id} className="card p-5 sm:p-6">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <h2 className="font-semibold text-foreground">{fraude.impostorDetails}</h2>
                    ...
                    <p className="text-sm text-foreground/90 whitespace-pre-line">{fraude.comments}</p>
                  </article>
                ))}
              </div>
            )}
            ...
            <div className="mt-10 text-center">
              <Link to="/reportar-estafa" className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">
